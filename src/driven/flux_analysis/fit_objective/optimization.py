...
"""Define functions and classes for optimization."""
...
# from driven.flux_analysis.fit_objective.generators import (
#     zero_one_binary_generator, zero_one_linear_generator)
# from driven.flux_analysis.fit_objective.variators import (
#     zero_one_binary_variator, zero_one_linear_variator)
...
def evaluate(model, candidates, coefficients, profiles, evaluators, **kwargs):
    ...
    if all(map(lambda v: v == 0, coefficients)):
        ...
        fitness_list = [f(model, coefficients, candidates, profile, **kwargs)
                        for f, profile in zip(evaluators, profiles)]
        coefficient_sum = sum(coefficients)
        return Pareto([coefficient_sum] + fitness_list)


def zero_one_bounder(candidate, args):
    """Get a zero or one as boundary."""
...
#     def __init__(self, profiles=None, evaluators=None, binary=True,
#                  use_reactions=True, model=None, heuristic_method=NSGA2,
#                  **kwargs):
#         self._heuristic_method = heuristic_method(Random())
#         self.model = model

#         if use_reactions:
#             self.candidates = [r for r in self.model.exchanges
#                                if r.lower_bound >= 0]
#         else:
...
#         return [evaluate(self.model, self.candidates, coeffs, self.profiles,
#                          self.evaluators, **self.kwargs)
