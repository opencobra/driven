...
"""Define classes and functions for results and analysis."""
...
# def _compare_flux_distributions(flux_dist1, flux_dist2, self_key="A",
#                                 other_key="B"):
...
# FluxDistributionResult.__sub__ = \
#     lambda self, other: _compare_flux_distributions(self, other)


# class FluxBasedFluxDistribution(FluxDistributionResult):
#     def __init__(self, fluxes, objective_value, c13_flux_distribution, *args,
#                  **kwargs):
#         super(FluxBasedFluxDistribution, self).__init__(
#             fluxes, objective_value, *args, **kwargs
#         )
...
#     def __init__(self, fluxes, objective_value, fba_fluxes,
#                  reaction_expression, cutoff, *args, **kwargs):
#         super(GimmeResult, self).__init__(
#             fluxes, objective_value, reaction_expression, *args, **kwargs
#         )
...
#             data,
#             index=index,
#             columns=["gimme_fluxes", "fba_fluxes", "expression",
#                      "inconsistency_scores"]
#         )

#     def reaction_inconsistency_score(self, reaction):
#         if reaction in self.expression:
#             return abs(self._fluxes[reaction]) * \
#                 max(self.cutoff - self.expression[reaction], 0)
#         else:
#             return 0

#     @property
#     def distance(self):
#         return sum([abs(self.fluxes[r]-self._fba_fluxes[r])
#                     for r in self.fluxes.keys()])

#     @property
#     def inconsistency_score(self):
#         return sum([self.reaction_inconsistency_score(reaction)
#                     for reaction in self.expression])
...
#             if abs(flux) == 0 and expression < self.cutoff and \
#                self.reaction_inconsistency_score(r_id) <= 0:
#                 model.reactions.get_by_id(r_id).knock_out(tm)


# class IMATResult(ExpressionBasedResult):
#     def __init__(self, fluxes, objective_value, expression, lower_cutoff,
#                  higher_cutoff, epsilon, *args, **kwargs):
#         super(IMATResult, self).__init__(
#             fluxes, objective_value, expression, *args, **kwargs
#         )
...
#    def data_frame(self):
#        index = list(self.fluxes.keys())
#        columns = ["fluxes", "expression", "highly_express",
#                   "lowly_expressed"]
#         data = zeros((len(self._fluxes.keys()), 4))
...
#     def __init__(self, flux_dist_a, flux_dist_b, a_key="A", b_key="B", *args,
#                  **kwargs):
...
#         assert all([rid in flux_dist_a.fluxes.index
#                     for rid in flux_dist_b.fluxes.index]) and \
#                all([rid in flux_dist_b.fluxes.index
#                     for rid in flux_dist_a.fluxes.index])
...
#         self._fluxes_a = {rid: flux/self._fluxes_a[reaction]
#                           for rid, flux in six.iteritems(self._fluxes_a)}
#         self._fluxes_b = {rid: flux/self._fluxes_b[reaction]
#                           for rid, flux in six.iteritems(self._fluxes_b)}
...
#         return sum(self._manhattan_distance(rid)
#                    for rid in self._fluxes_a.keys())
...
#         return sqrt(sum(self._euclidean_distance(rid)
#                         for rid in self._fluxes_a.keys()))
